...
class Canvas
{
    constructor(id)
    {
        ...
        this.canvas.imgData.data.fill(255);
        this.canvas.ctx.putImageData(this.canvas.imgData, 0, 0);
        this.canvas.tempImgData = null;

        this.canvas.chosenTool = "line";
        this.canvas.chosenColor = new Color(0, 0, 0);
        this.canvas.brushRadius = 10;
        this.canvas.sprayRadius = 10;

        this.canvas.sprayTimer = new Timer(20, null);
        ...
        function onMouseDown(e)
        {
            ...
            if (e.button == "0")
            {
                ...
                {
                    case "flood_fill":
                        if (!e.currentTarget.getPixel(e.currentTarget.tempImgData, e.currentTarget.p1.X, e.currentTarget.p1.Y).equals(e.currentTarget.chosenColor))
                        {
                    ...
                    case "pencil":
                        e.currentTarget.drawPoint(e.currentTarget.tempImgData, e.currentTarget.p1.X, e.currentTarget.p1.Y, e.currentTarget.chosenColor);
                    ...
                    case "brush":
                        e.currentTarget.fillCircle(e.currentTarget.tempImgData, e.currentTarget.p1.X, e.currentTarget.p1.Y, e.currentTarget.brushRadius, e.currentTarget.chosenColor);
                    ...
                    case "spray":
                    {
                        ...
                        { 
                            e.srcElement.sprayOnce(e.srcElement.tempImgData, e.srcElement.p1.X, e.srcElement.p1.Y, e.srcElement.sprayRadius, e.srcElement.chosenColor); 
                        
                            e.srcElement.imgData = e.srcElement.ctx.createImageData(e.srcElement.tempImgData);
                            e.srcElement.imgData.data.set(e.srcElement.tempImgData.data);
                            
                            e.srcElement.updateCanvas(e.srcElement.imgData);
                        };

                        e.currentTarget.sprayTimer.Callback = callback;

                        e.currentTarget.sprayTimer.start();
                    }
                    break;
                }
            }
        ...
        function onMouseUp(e)
        {
            if (e.button == "0")
            {
                e.currentTarget.sprayTimer.stop();
            ...
            if (e.currentTarget.isMouseDown)
            {
                ...
                switch (e.currentTarget.chosenTool)
                {
                    ...
                    case "spray":
                        if (e.currentTarget.tempImgData != null)
                        {
                            e.currentTarget.spray(e.currentTarget.tempImgData, e.currentTarget.p1.X, e.currentTarget.p1.Y, e.currentTarget.p2.X, e.currentTarget.p2.Y, e.currentTarget.sprayRadius, e.currentTarget.chosenColor);
        ...
        this.canvas.fillCircle = function(imgData, x0, y0, R, color)
        {
            ...
            while (v < 10)
            {
                ...
                this.drawLine_Bresenham(imgData, x0 - y, y0 + x, x0 + y, y0 + x, color);
                this.drawLine_Bresenham(imgData, x0 - y, y0 - x, x0 + y, y0 - x, color);
                this.drawLine_Bresenham(imgData, x0 - x, y0 - y, x0 + x, y0 - y, color);
            }
        }

        this.canvas.sprayOnce = function(imgData, x0, y0, R, color)
        {
            for (let i = 0; i < Random.next(15, 31); i++) 
            {
                let x = 0;
                let y = 0;
                ...
                do 
                {
                    x = (Random.nextDouble() * R * 2) - R;
                    y = (Random.nextDouble() * R * 2) - R;
                } 
                while ((x*x + y*y) > (R * R));
                ...
                this.drawPoint(imgData, Math.floor(x), Math.floor(y), color);  
            }
        }

        this.canvas.spray = function(imgData, x0, y0, x1, y1, R, color)
        {
            let deltaX = x1 - x0;
            let deltaY = y1 - y0;
            ...
            for (let i = 0; i < distance; i++) 
            {
                ...
                this.sprayOnce(imgData, Math.floor(ipos.X), Math.floor(ipos.Y), R, color);
            }
        }
    ...
    get BrushRadius()
    {
        return this.canvas.brushRadius;
    }

    set BrushRadius(value)
    {
        this.canvas.brushRadius = (value < 1) ? 1 : ((value > 200) ? 200 : value);
        this.canvas.brushRadius = Math.round(this.canvas.brushRadius);
    }

    get SprayRadius()
    {
        return this.canvas.sprayRadius;
    }

    set SprayRadius(value)
    {
        this.canvas.sprayRadius = (value < 1) ? 1 : ((value > 200) ? 200 : value);
        this.canvas.sprayRadius = Math.round(this.canvas.sprayRadius);
    }
